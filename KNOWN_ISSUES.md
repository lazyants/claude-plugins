...
- **Confirmed (2026-04-24):** `/remote-control` slash command. Responds with `Unknown command: /remote-control` while both vars are set; returns after both are removed from `~/.claude/settings.json` `env` block and Claude Code is restarted. Shell-level `env -u` is insufficient — `settings.json` values override. Remove the JSON keys, then restart.

  **Individual attribution unverified.** Both vars were applied and removed together. A future test can isolate which of the two is solely responsible (or whether it takes both) by removing one at a time. Candidates in order of suspicion: `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` (documented kill-switch for all non-essential features) > `DISABLE_TELEMETRY` (narrower — but upstream #34178 shows it also gates GrowthBook).
