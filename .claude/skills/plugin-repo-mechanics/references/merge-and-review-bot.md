...
- **A `NEUTRAL` check conclusion is ADVISORY, not merge-blocking.** The bot posts `CHANGES_REQUESTED` with inline findings, yet `reviewDecision` stays `null` and `mergeStateStatus` sits at `UNSTABLE`; the PR is still `MERGEABLE`. This repo has CI now, so read WHICH check is `UNSTABLE`: a failing or still-running workflow run reads the same in that one field. Don't read UNSTABLE/CHANGES_REQUESTED as "blocked" — but resolve it (below) for a clean merge. It re-reviews automatically on push: after a fix commit the check flips `IN_PROGRESS`→done (~3–4 min); zero new inline comments on the new head = satisfied.
