...
that needed it — same failure shape as zsh does not word-split an unquoted `$VAR` (→ personal skill:cc-harness-ops — global, not in this repo). Count NUL records, not
...
the Bash tool's cwd is not stable across builds (→ personal skill:cc-harness-ops — global, not in this repo) and the standing "always use an isolated working copy" rule,
