...
- **Cheap garble detector — the single-letter-Hebrew-token ratio.** `clean ≈ 2–3%`, `garble ≈ 55–60%` (with the strong predicate below — the weak `len(token)==1` form reports the same garbled corpus as ~14%). Run it on the extractor output AND on any prior run's EPUB you're tempted to reuse. NEVER reuse a previous project's EPUB without this check: the SSK July first-run EPUB was a 59%-garbled `pdftotext` dump, and slicing it would have carried the garble straight into a "clean" re-run.
  - **The check applies to ANY already-extracted corpus you are about to build on — including an existing `manifest.json` that already passed W2.** `validate_extraction.py` checks structure and conservation, NOT token integrity, so a green W2 gate is **not** evidence of clean text. Measure the manifest's own `plain_text`, not just a fresh extractor run. (Cost of skipping it: a full 40-segment LLM name canon — discovery, map-reduce consolidation, six verification rounds, two scope purges — was built on a manifest later measured at **59% single-letter** — fully garbled, not marginal — before anyone checked it.)
  - **COUNT HEBREW LETTERS INSIDE THE TOKEN, NOT `len(token)` — the weak predicate under-reports by 4×.** A vocalized single letter (`אַ` = letter + combining mark) has ONE Hebrew letter but Python length 2, so `len(t) == 1` misses most shredded tokens. Use the strong predicate:
    ```python
    ...
    single = sum(1 for t in heb if sum(1 for c in t if is_heb_letter(c)) == 1)
    ```
    This matters for the interpretation, not just the number: the SSK manifest measured **14.1% under `len(t)==1` but 59.3% under the strong predicate** — i.e. it was in the *garble* band all along, and an earlier revision of this rule wrongly concluded from the weak figure that "the bands are not bimodal, an intermediate ~14% is still degraded". **With the strong predicate the bands ARE bimodal** — clean 2.1–2.9%, garble 59% — a 20× gap with nothing in between. If you land in a genuine middle, suspect your predicate before inventing a band.
  - **Pair it with a second signal, also with the strong predicate.** Count combining marks whose predecessor is **neither a Hebrew letter nor another Hebrew mark** — not merely those following a space, which misses marks after a newline or punctuation. Measured on the same book: **~1 per 26 Hebrew tokens clean vs ~1 per 10 degraded** (the space-only variant gave 1-per-31 vs 1-per-11 — same direction, weaker instrument).
  - **State both gates as integer arithmetic and pin test vectors.** `20*single <= len(heb)` and `20*detached <= len(heb)` (≤5% each), asserting `len(heb) > 0` first. A prose threshold ("no more than 1 per 20 tokens") reads as a rate and inverts easily — that phrasing shipped an inverted gate that would have *failed* clean text and *passed* degraded. Pin vectors for: vocalized `אַ` (counts as single-letter), `ב-` (not a Hebrew token), mark-after-newline and mark-after-punctuation (both detached), mark-after-its-base-letter (not detached).
