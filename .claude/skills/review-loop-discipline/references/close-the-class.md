...
**A refusal test that asserts only the OUTCOME cannot attribute the refusal to your guard.** In a fail-closed module every guard produces the same coarse result — `ok: false`, one halt class — and so does an upstream guard that fired first, and so does a *crash* inside the guard you were trying to pin. Three measured instances on one branch, all green, all worthless: a test written for an unresolvable-root arm returned the IDENTICAL halt under production and under its own mutant, because a guard several frames up refused first (it was deleted rather than kept); a second survived outright DELETION of its guard, because the deletion raised a TypeError that the module's own catch converted into the same halt class; a third passed because a neighbouring identity pin caught the input, not the bracket under test. **Assert the DIAGNOSTIC — the message, the reason slug, the subject it names — never the class,** and establish it by running the guard's ABSENCE and diffing what changes, not by reading the code. Fail-closed by crash is not the same fact as fail-closed by adjudication.

**The same measurement settles the sibling claim, which is an attribution claim wearing different clothes: "this branch is unreachable."** It is the claim most often made from the paths you happened to open, and it licenses deleting a real test. Twice on that branch it was wrong, and what settled it both times was COUNTING the observations rather than re-reading them — the same path turned out to be resolved twice by different call sites, so a window existed between them that no amount of tracing had suggested. Before calling a branch unreachable, instrument the input and count who touches it.

**Corollary for the mutation harness itself: a mutant that changes the file's sha has not necessarily changed its SEMANTICS.** One "applied" cleanly by inserting `if (false) {}` ahead of a comment, disabled nothing, and killed no tests — which reads exactly like a coverage gap and sends you off to write a fixture for a guard that was never removed. Assert that the mutant *disabled* something, not merely that the bytes differ.
