...
never `wc -c`, when the text is not ASCII. Sibling of zsh does not word-split an unquoted `$VAR` (→ personal skill:cc-harness-ops — global, not in this repo) — same fix
