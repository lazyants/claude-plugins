...
description: Working ON the enduser-handbook plugin — modifying its skill, publish-target/adapter-filename resolution, link-emission canons (wikilinks/index rows), adding or removing a profile.yml key, grep assertions and enumeration sweeps over its hard-wrapped docs (`has` vs `has_in_section`, `hasnt_joined` vs `hasnt_joined_code`, "must-now-fail" self-tests, reference-assets.test.sh portability including its Ruby- and esbuild-gated assertion counts, a subtree `git archive` test bed's absolute counts), chapter-paths delimiter tests, findStaleChapterRows exemption logic, a title-refusal asymmetry between path and wikilinks modes, or partitioning a multi-issue backlog around chapter-paths.mjs, citation-audit-lib.mjs regex, a stale `file:NNN` code citation in these docs, profile-schema-evaluator.mjs `in` vs `Object.hasOwn`, tracing what a gate/backstop really catches, a halt-driven manual recipe completion check, its capture-safety PII audit, the capture-guard redirect-hop audit's detection-vs-prevention scope, or a declaration/runtime parity (.d.mts) pin.
---
...
- **`references/capture-guard-facts.md`** — read before touching `capture-guard-policy.mjs`/`capture-spec-helpers.md` or their copyable example fixtures: the redirect-hop audit's detection-only scope, and the duplicate-skeleton and `object[data]` false claims that shipped into them.
- **`references/declaration-parity-facts.md`** — read before touching `declaration-parity.test.mjs`/`export-parity-lib.mjs` or the three type-only census needles in `reference-assets.test.sh` — a settled decision, re-proposed twice already.
- **`references/chapter-paths-exemption-checklist.md`** — read before touching `findStaleChapterRows` or the flat-append title-refusal logic: the five-shape exemption checklist, the #585 hard-halt rationale, and the title-refusal asymmetry that shipped as a false universal.
...
- **A SECOND, independent gate on the same total: `reference-assets.test.sh:3106`'s profile-schema validation assertion is Ruby-gated.** It is emitted only when `command -v ruby && ruby -ryaml -rjson -e '1'` succeeds; the `else` branch prints a note and emits **zero** assertions instead of a failing one, so the total moves silently rather than going red. macOS ships `/usr/bin/ruby`; a Linux CI image typically does not. Measured 2026-08-18 on 1.18.1: the bot's sandbox read 1080 → 1081 where the local machine read 1081 → 1082 — proved by shadowing `ruby` with a stub that exits 1, which reproduced the bot's exact lower total. Before attributing a suite-total mismatch to the esbuild gate above, check this one too: a first hypothesis that blamed the esbuild gate for a mismatch this Ruby gate actually caused was wrong and shipped from a half-remembered note before being corrected. The general rule, confirmed against literary-translator's own suite as a cross-check: a count is portable when a gate suppresses EXECUTION (an `if`-guarded assertion block, like both gates here) and NOT when it suppresses COLLECTION (`pytest.importorskip`, `pytest.skip(..., allow_module_level=True)`, `collect_ignore` — none of which this bash suite has an analogue for, since it has no separate collection phase).
