...
description: Working ON the enduser-handbook plugin — modifying its skill, publish-target/adapter-filename resolution, link-emission canons (wikilinks/index rows), adding or removing a profile.yml key, grep assertions and enumeration sweeps over its hard-wrapped docs (`has` vs `has_in_section`, `hasnt_joined` vs `hasnt_joined_code`, "must-now-fail" self-tests, reference-assets.test.sh portability and its Ruby/esbuild-gated assertion counts, a subtree `git archive` bed's absolute counts), chapter-paths delimiter tests, findStaleChapterRows exemption logic, a title-refusal asymmetry between path and wikilinks modes, partitioning a multi-issue backlog around chapter-paths.mjs, citation-audit-lib.mjs regex, a stale `file:NNN` citation in these docs, profile-schema-evaluator.mjs `in` vs `Object.hasOwn`, tracing what a gate really catches, a halt-driven manual recipe check, its capture-safety PII audit, the capture-guard redirect-hop audit's detection-vs-prevention scope, or a declaration/runtime parity (.d.mts) pin.
---
