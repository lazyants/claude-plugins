...
description: Working ON the enduser-handbook plugin — use when modifying its skill, changing publish-target/adapter-filename resolution or a link-emission canon (wikilinks/index rows), tracing whether a gate/backstop claim actually catches what it claims to, writing or debugging grep-based test assertions or enumeration sweeps over its hard-wrapped SKILL.md/references docs, writing chapter-paths delimiter/threshold tests, testing reference-assets.test.sh for cross-shell/cross-env portability, writing a "must-now-fail" self-test, touching citation-audit-lib.mjs's regex matching, changing an object-membership check in profile-schema-evaluator.mjs (`in` vs `Object.hasOwn` and the prototype-chain leak), designing a completion/convergence check for a halt-driven manual-work recipe, or hardening/auditing its capture-safety surface-audit against PII leaks.
---
...
**The scope is any plain object whose keys come from OUTSIDE the code — not just the evaluator.** Wherever keys are filenames, slugs, user config keys or any other external namespace, a plain `{}` is wrong in *two* directions, and the second one is not a membership bug at all: **assigning to `__proto__` is silently dropped**, so `map['__proto__'] = v` leaves no own property and `JSON.stringify` emits `{}` — a value lost on WRITE, with no error anywhere and nothing for a membership fix to catch. A screenshot may legally be named `toString.png` or `__proto__`; both were live against 1.12.0's provenance hash maps. Use a `Map` or a null-prototype object for the container **and** `Object.hasOwn` for membership; one without the other still ships one of the two directions.
