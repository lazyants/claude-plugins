...
**The same runner also reports a CRASHED file as a clean pass — only the exit code disagrees.** A `ReferenceError` thrown at MODULE scope (e.g. a `test()` title interpolating a `const` that a refactor deleted) is caught by the runner as an *uncaught exception* and surfaces as one buried line in the middle of stdout — `# Error: A resource generated asynchronous activity after the test ended … "ReferenceError: X is not defined"` — while the summary still prints `# pass 433 / # fail 0`. Measured 2026-07-27: exit code `1`, counters pristine. Reading the counters is the natural verification and it is the one that lies; the tail of the output looks exactly like a healthy run.

**Apply: bank a `node --test` result on its EXIT CODE, never on `# pass`/`# fail`.** `node --test <file> >/dev/null 2>&1; echo $?` is the whole check. Note this is the mirror image of the shell trap in §5(a) below — there a `FAIL` printed while the suite exited 0; here a clean count printed while the process exited 1 — so the general rule is **counters and exit status are independent signals and either can be the honest one; require both to agree**. A wrapper that branches on the status (`if node --test "$t"; then ok; else bad; fi`) is immune and is why this repo's `reference-assets.test.sh` caught it when a human reading the counters did not.
