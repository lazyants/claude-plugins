...
**A crashed file's failure is easy to MISS in this runner's output, though the counters do report it.** A `ReferenceError` from a `test()` title interpolating a `const` a refactor deleted, or one thrown asynchronously after a test ended, surfaces as a single line buried in the middle of stdout — `# Error: A resource generated asynchronous activity after the test ended … "ReferenceError: X is not defined"` — while everything around it, and the whole tail, looks like a healthy run. Skimming to the summary is the natural move and it is where the eye lands.

**A claim retracted here, because the retraction is the lesson.** An earlier revision of this note asserted the counters stay *pristine* (`# pass 433 / # fail 0`) while the process exits 1. That does not reproduce. Re-measured 2026-07-27 on **Node v22.23.1**, twelve passing tests plus one faulting test per file: a module-scope `ReferenceError` gives `tests 13 / pass 12 / fail 1`, exit 1; an async-after-test `ReferenceError` gives `tests 14 / pass 13 / fail 1`, exit 1. The `lazy-ants-reviewer` bot independently ran both probes on **v22.23.1 and v26.3.0** and got a nonzero failure count every time. Counters and exit status agreed in all four runs. Whatever produced the original `433 / 0` observation was not the runner, and it was written down without a re-derivation path — so treat any "the counters lie" claim as unverified until you have the exact command and its output.
