...
- **Declaring a carve-out property is what removes the `unevaluatedProperties` barrier that had been rejecting it.** A merge step read an untrusted fragment with NO schema validation and copied its keys verbatim into a materialized ledger entry; before the field existed in the schema, an `unevaluatedProperties: false` gate silently rejected a planted `stale_mismatched_fields` key on that fragment. Adding the field to `ledger.schema.json` to support a legitimate carve-out reopens exactly that hole — the schema no longer treats the key as unevaluated, so a forged value now validates and merges straight through. The fix has to close the hole in the SAME change that opens it: strip the field unconditionally before the real diff is computed, never trust a value that arrived through an unvalidated read. Ask this of every `then`/`allOf` branch that legitimizes a new property under `unevaluatedProperties: false`: does anything upstream write that property WITHOUT going through this schema first? Source: `project-lt-491-pr494.md`.
