...
A GREEN certifies only what the gate can see; a false-GREEN silently passes your OWN output's defect. Four concrete under-catch mechanisms (the first two from a format+completeness gate):
...
3. **"Did the VALUE change?" is the wrong signal for "did we ACT?" — gate on "was there anything to RESOLVE".** A resolve-then-report function that decides it transformed nothing by comparing `text == original` false-negatives every **no-op substitution**: a known token that legitimately maps to itself (X→X) leaves the string byte-identical, so the function reports "no transform" and skips the follow-up it owed. Gate instead on whether a known token MATCHED — the question actually being asked. Found by code review on literary-translator 1.4.2, in the renderer's **#171** heading-sentinel invariant (`render_obsidian.py:2245-2254`, `:2297`) — NOT #173, which is the separate exact-map placeholder-fidelity fix in the same release.
4. **Per-unit review is structurally blind to cross-unit drift.** N independently-generated units each pass their own per-unit review "clean" yet drift in cross-unit PRESENTATION (one segment prefixes source lines with a `[Yiddish]` label; the other 39 use a bare `>`). The per-unit review CAN'T catch this — it never sees the others. Only a corpus-wide normalization gate over the assembled whole does.
...
1. **out_dir itself a symlink** → an iterate-and-remove clean (`iterdir()` / `os.walk`, then `unlink`/`rmtree` each child) lists and deletes the TARGET's contents. Refuse if `out_dir.is_symlink()`, and check it BEFORE `mkdir(exist_ok=True)` (mkdir happily accepts a symlinked dir as "exists").
2. **A symlink ENTRY inside out_dir** → `entry.is_dir()` follows it, so a naive `if entry.is_dir(): rmtree(entry)` targets the link. But `shutil.rmtree()` on a symlink does NOT delete-through — it **RAISES `OSError` (documented "Cannot call rmtree on a symbolic link")** — so the harm is a crash mid-clean (non-deterministic cleanup / DoS), NOT silent data loss. Still check `entry.is_symlink()` BEFORE `is_dir()` and `unlink()` the symlink (remove the link, never its target) so the clean completes deterministically. **Neither `shutil.rmtree(link)` nor `rm -rf link` deletes THROUGH a directory symlink** — both remove only the link itself (`rm -rf` unlinks it; `rmtree` raises). A genuine delete-THROUGH needs either an EXPLICIT follow-links traversal — `os.walk(followlinks=True)` + a descending `os.unlink`/`os.remove` — or the container-level follow of vector 1 (iterating / `os.walk`-ing the SYMLINKED out_dir ITSELF, whose listing is the TARGET's entries).
