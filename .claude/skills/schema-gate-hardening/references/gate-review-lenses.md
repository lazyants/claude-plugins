...
3. **"Did the VALUE change?" is the wrong signal for "did we ACT?" — gate on "was there anything to RESOLVE".** A resolve-then-report function that decides it transformed nothing by comparing `text == original` false-negatives every **no-op substitution**: a known token that legitimately maps to itself (X→X) leaves the string byte-identical, so the function reports "no transform" and skips the follow-up it owed. Gate instead on whether a known token MATCHED — the question actually being asked. Found by code review on literary-translator 1.4.2, in the renderer's **#171** heading-sentinel invariant (`render_obsidian.py:1838-1849`, `:1890`) — NOT #173, which is the separate exact-map placeholder-fidelity fix in the same release.
