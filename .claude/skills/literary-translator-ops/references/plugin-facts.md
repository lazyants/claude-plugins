...
- **A `"default": <value>` annotation in `profile.schema.json` is documentation-only — nothing fills it
  ...
  **three** call sites: `validate_backlinks.py:521` (`_effective_enabled`), `assemble.py:3157`
  (`_effective_mentions_enabled`), `render_obsidian.py:255` (`_effective_mentions_enabled`) — each
