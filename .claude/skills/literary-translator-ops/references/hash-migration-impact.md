...
Belt-and-braces: `CANON_ENTRY_FIELDS` (`canon_validate.py:689-691`) is the projection
...
(`_stamp_write_verify`, `canon_validate.py:2692-2697`; callers `run_merge` / `run_merge_batches` /
...
`run_restamp_derivation` (`canon_validate.py:2860-2900`) re-records the CURRENT `particle_config_hash` /
...
unconditionally (`canon_validate.py:2692-2711`, "a merge that changes nothing no longer moves
