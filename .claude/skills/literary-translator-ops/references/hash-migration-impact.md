...
(`compute_derivation_bundle_hash`, `cache_key.py:662-665`) — no comment-stripping, so even a comment
reword flips it.
...
(`diff_rendered_output.py:194`, hashed `:335`, stale-check `:599`). Editing either flips it. On the next
...
`canon.json` **content**: `used_terms_hash` (15-field cache-key field #3, `cache_key.py:765-771`) hashes
...
Belt-and-braces: `CANON_ENTRY_FIELDS` (`canon_validate.py:690-692`) is the projection
...
(`_stamp_write_verify`, `canon_validate.py:2980-2985`; callers `run_merge` / `run_merge_batches` /
...
`run_restamp_derivation` (`canon_validate.py:3175-3215`) re-records the CURRENT `particle_config_hash` /
...
unconditionally (`canon_validate.py:2980-2999`, "a merge that changes nothing no longer moves
