...
3. **Duplicate test.** Key on the function and its branch, never on the title or the input. One broken branch is one row. Two open numbers on one branch attract opposite remedies, and that — not the noise — is the damage.
