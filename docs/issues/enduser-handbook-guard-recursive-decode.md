...
- The author's `denyPatterns` run first and catch known dangerous shapes regardless of encoding nuance.
- Every non-GET request fails closed (`[guard:fail-closed]`), so a doubly-encoded **write** is still blocked.
- The gap is narrow: a destructive **GET** whose verb is doubly-encoded, against a server that double-decodes.
