...
- [Additional publish-target adapters](issues/enduser-handbook-publish-adapters.md) — Confluence / GitBook / Docusaurus adapters beyond the shipped `obsidian-vault` + `static-md`.
...
- [Capture-doc accuracy trims (forkability wording + GraphQL-only read-hatch)](issues/enduser-handbook-capture-doc-accuracy.md) — doc-only: "fork it for other engines" oversells transferring the Playwright `.ts` asset, and the shipped read-hatch classifier is GraphQL-only so REST/Django POST-reads fail closed; deferred from 1.1.0. _(PR #13, 2026-06-21)_
