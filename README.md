...
Trigger phrases: "corrupted installLocation", "claude plugin across profiles", "CLAUDE_CONFIG_DIR profiles", "plugin deleted in one profile", "multi-profile plugins", "CODEX_HOME", "second codex account", "codex profile reads the wrong home" — full lists in `plugins/multi-profile-plugins/skills/multi-profile-plugins/SKILL.md` and `.../skills/multi-profile-codex/SKILL.md`.
