...
### Fixed
- **`wireNestedListChapter` unbounded index-row growth** — a runtime behavior fix to the shipped 1.10.0 nested-list writer, not a documentation correction. The writer had no membership check of its own: it relied entirely on the caller's step-0 presence scan (`locateChapterLine`), which recognizes a row by parsing a link target out of it. A manifest `title` whose text keeps its own row's link destination from parsing (for example, a literal `]`) is therefore reported absent by step 0 on every run, so the insert-only writer appended the same row again on every publish — unbounded index growth from a legal manifest value, with no operator mistake anywhere. `wireNestedListChapter` now runs its own literal, content-verbatim membership check and returns `{kind: 'present', index}` — one of the two outcomes 1.11.0 adds to the shipped 1.10.0 trio (`inserted | multiple | not-a-list`), making the returned union five: `inserted | present | unwritable | multiple | not-a-list` — instead of inserting a duplicate whenever the target container already carries this exact chapter link, so a caller can stop instead of silently appending forever. Scoped to the marker it holds for: this is what bounds the `-`-markered case, where such a title inserts and would otherwise append forever; on `*`/`+` the same title never reaches the file, for the separate reason below.
- **The 1.10.0 writer could poison the index file it wrote into** — the second runtime behavior fix, and the one with the wider blast radius. 1.10.0 emitted whatever the caller's `group_title` and `chapterLink` produced, with a plain-label check on the container label and none at all on the child row, so a manifest value that is legal everywhere upstream was written into a file that was clean a moment earlier — and **that same scanner then refused the file on every later run**. Nested-list automation died for every chapter and every group in that index, not only the one that caused it, and the operator saw only the generic `not-a-list` manual halt, which names the chapter it is currently trying to wire and never the row that broke the file. 1.11.0 gates emission on a re-read postcondition and returns the other new outcome, `{kind: 'unwritable', field}`: before handing anything back, the writer joins the exact bytes it is about to persist and feeds them through its own reader — the same `prepareIndexLines` / `hasYamlMappingStructure` / `containerOwnerScan` pipeline the next run would use, deliberately not a second copy of that pipeline's rules, since a copied rule cannot notice a rule it never copied — and if that reader would decline the result, nothing is written at all. `field` is derived by substitution rather than by parsing the value (swap one emitted line for a known-recognizable stand-in, re-read, see whether the rejection clears), so it names `'title'`, `'group_title'`, or `'unknown'` when both values are independently at fault, and stays correct for causes not yet catalogued. Both adapters surface it as a halt naming that field; the halt string is defined once in the adapters and pinned there byte-for-byte, so it is referenced rather than duplicated here. The fatal set is a matrix over field × link mode × the bullet marker **of the line being written** — not of the file, which a mixed-marker index makes visible: the same title is refused under a `*`-marked container and written under a `-`-marked one in that same file. In a `title`, on any marker: a backtick (any run length, paired or not — the mechanism is an unterminated inline code SPAN, never a fence, and a tilde run is measured harmless), an HTML comment, or a U+2028/U+2029 line separator. Under a `*`/`+`-marked container additionally anything that keeps the row's own link target from parsing, which then trips `isBarePathBullet` on the grouped target's `/`: in path mode an unescaped `]` or a trailing odd run of backslashes, and in wikilink mode a `]` whether or not it is backslash-escaped, since `WIKILINK_TARGET_RE` is not escape-aware. In a `group_title`: a U+2028/U+2029 on any marker, a `/` anywhere or a terminal `.md` under `*`/`+`, and a colon straight after the first token or an all-hyphen value under `-`. Inline code or an HTML comment in a `group_title` is **not** in this set — it returns `not-a-list` and takes the pre-existing manual halt, short-circuiting before a write is ever attempted. Stated as a rule rather than a count, deliberately: four earlier attempts to state this in prose were each measured false, and a bare cell count is not reproducible without the value list that produced it. Measured on both releases for every cell named above: 1.10.0 wrote them and the resulting file then answered `not-a-list` even for an unrelated group; 1.11.0 writes none of them. On both axes the index bytes are unchanged and the halt repeats identically on every re-run rather than locking anything — but what still wires alongside it differs by field, so it is stated per field rather than flattened into one sentence. On a `title` fault, every other chapter on that untouched file still wires, including another chapter under the very container that failed. On a `group_title` fault the fatal value **is** the container label, so every chapter in other groups still wires while every chapter under that same container keeps returning the same `unwritable`/`group_title` until the manifest value is fixed — the intended refusal to build a container the next run could not read, not a residual lockout: the file is never modified, and correcting the value wires the chapter on the very next run against those same unchanged bytes.
