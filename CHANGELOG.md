...
- **Convergent #329 halt** — the shipped `not-a-list` manual halt in both adapters now also tells the operator exactly what shape the next run recognizes (a Markdown list row indented two spaces under the named container, with the exact link or wikilink target) for a `group_title`, target and title an isolated gate accepts, so one manual edit converges on the very next run for that case — provided the row's title RENDERS as plain text in the same sense `group_title` must, which is the rendered form rather than the raw characters (a backslash escape decodes, so `A\.B` counts as plain). A title that renders non-plain leaves the row unverified rather than caught; a title whose markup keeps the row's own link target from resolving is not recognized at all, so the chapter is reported absent again and the same halt repeats rather than the run completing. The halt therefore asks for a plain-text title outright: a markup title placed correctly under the container is not refused, it is inserted alongside — 1.10.0 behaviour this release neither introduces nor fixes. A triple the gate rejects (an embedded newline, a padded `group_title`, a disallowed character in the target, ...) gets the unchanged 1.10.0 halt instead, with no improvement and no convergence claim. #329 itself — automating a bare path-table container — is closed as not soundly automatable rather than delivered; this halt improvement is what the issue gets instead.
