...
- 462 unit tests across the plugin's suite (`plugins/enduser-handbook/tests/*.test.mjs`), of which the #220/#221 changes in `chapter-paths.test.mjs` account for 168; 387 documentation assertions in `reference-assets.test.sh` (366 pinning doc/source content, 21 self-testing the `has_in_section`/`hasnt_in_section` engine itself).
