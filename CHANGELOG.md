...
### Testing
- `tests/reference-assets.test.sh` gains `has_in_section`, bounding a fixed-string assertion to one Markdown section (heading to the next same-or-shallower heading, one-pass `awk`, fixed-string `index()` matching). Whole-file greps could not distinguish a group-free branch from its already-correct grouped sibling — the existing embed-formula needle passed while the group-free glossary branch was still wrong. It follows CommonMark fence rules (a needle sitting only inside a fenced code block is not proof — fence content is skipped), normalizes CRLF before both the heading compare and the needle scan, and hard-fails on an absent heading or an empty needle. HTML comments are deliberately **not** tracked — a needle inside a `<!-- ... -->` still counts as found — after two codex rounds found real regressions in an earlier comment-aware state machine; every current caller's needle sits in plain prose, so the gap doesn't bite today.
...
<!-- Re-verify this line's counts against a fresh `node --test .../*.test.mjs` + `bash .../reference-assets.test.sh` run after the final clean review verdict, before commit — review rounds routinely add assertions after this was first written. -->
- 446 unit tests across the plugin's suite (`plugins/enduser-handbook/tests/*.test.mjs`), of which the #220/#221 changes in `chapter-paths.test.mjs` account for 152; 325 documentation assertions in `reference-assets.test.sh` (323 pinning doc/source content, 2 self-testing the `has_in_section` helper itself).
