...
You walk it in the other direction too: every label a **captured** step quotes verbatim has a capture-time assertion pinned to that exact string, scoped narrowly enough to identify the one instance the step narrates, so a rename fails the capture run instead of leaving a step whose quoted label is absent from its own screenshot. That covers button and menu captions, and equally the column headers, field labels, tab captions and empty-state copy the prose names. A visibility assertion on the element is not that pin; see step 4 of `page-identity.md`.

**Disclosure prose is the exception, and deliberately so.** The obligation attaches to a numbered step, which by definition has a capture behind it. A label quoted only in disclosure prose has no such step — the action was never fired, or the capture role or the seed data never rendered the control at all — so there is nothing at capture time to pin it to. It is still verbatim: source it from the running UI per `running-ui-source.md` wherever some role or state can show it, and where none can, say in the prose where the label came from. The boundary runs through the label, not through the step: a disclosure that embeds a captured open state still pins every label that screenshot documents, and excuses only the one the run never reached. For a *captured* step, a quoted label with nothing pinning it is an untraced step, and it takes the same three outcomes as any other trace failure: re-capture it, demote it to disclosure prose, or delete it.
