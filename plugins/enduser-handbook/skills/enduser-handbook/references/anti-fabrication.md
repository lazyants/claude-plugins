...
You walk it in the other direction too: every label the chapter quotes verbatim — button and menu captions, and equally the column headers, field labels, tab captions and empty-state copy the prose names — has a capture-time assertion pinned to that exact string, scoped to the region the shot captures, so a rename fails the capture run instead of leaving a step whose quoted label is absent from its own screenshot. A visibility assertion on the element is not that pin; see step 4 of `page-identity.md`. A quoted label with nothing pinning it is an untraced step, and it takes the same three outcomes as any other trace failure: re-capture it, demote it to disclosure prose, or delete it.
