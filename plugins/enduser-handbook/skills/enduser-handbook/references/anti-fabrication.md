...
**Disclosure prose is the exception, and deliberately so.** A control you could not capture has no screenshot for a label to be absent from and no capture step to hang an assertion on, so the obligation above does not reach it — that is the whole point of the disclosure shape described earlier in this file. Its quoted label is still verbatim, verified against the running UI per `running-ui-source.md` rather than against a PNG. For a *captured* step, a quoted label with nothing pinning it is an untraced step, and it takes the same three outcomes as any other trace failure: re-capture it, demote it to disclosure prose, or delete it.
