...
**What is general here, and what is the shipped instance of it.** Every step below is written in files and paths, because both adapters that ship publish into a filesystem. The OBLIGATION is not: it is that a group change relocates the chapter, its assets, its provenance record and its index membership, that inbound references to the old location are repaired, that the capture sink follows, and that none of it counts as done until terminal state proves it. The steps below are that obligation's static/vault instance, not the obligation itself. A target with a different object model — pages under a parent, attachments on a page — owes the same seven things in its own coordinates and owes them nowhere else, since nothing in this file will name them. `references/publish-targets/README.md` ("Revalidation and W6 integration") requires each adapter to restate these steps for its own target and to say explicitly which have no analogue: an operator reading a step that names a file path for a target with no files cannot otherwise tell a deliberate omission from a forgotten one. The same reading applies to the terminal-state checklist below — its twelve fact kinds are derived from the manifest entries plus `publish.chapters_dir` and `capture.output_dir` — eleven carrying a path, an index target or a capture-spec spelling, and `title-container` carrying `group_title` alone — so they are the file-target instance of "what proves this migration finished", and a non-file target's own facts are prose that this plugin cannot yet emit or verify (that limit, and why it is a limit rather than an oversight, is stated in the same README section).
...
2. Move the asset directory from its old path to its current derived path — `capture.output_dir/<slug>/` flat, `capture.output_dir/<group>/<slug>/` grouped. Do NOT re-run capture for this move: a group-only relocation is not a feature change, so the existing screenshots are still correct — see the recapture carve-out in `manifest-discipline.md`. Move the provenance record alongside it, when this run's W1 ownership outcome was active (1.12.0) — from `<publish.chapters_dir>/.provenance/chapters/<old-group>/<slug>.json` to `<publish.chapters_dir>/.provenance/chapters/<new-group>/<slug>.json`, both full paths named by the halt below. A skipped run never had a record to move.
3. Update the index file: retarget or remove the old line, then wire the current line under the correct container, creating the container (a `##` heading, or a bullet for a nested-list index) first if it does not exist yet — the same container-resolution wiring the adapter uses when establishing a brand-new grouped chapter.
...
7. For a removed entry: delete its chapter file, its asset directory, its index line, and — when this run's W1 ownership outcome was active (1.12.0) — its provenance record at `<publish.chapters_dir>/.provenance/chapters/<group>/<slug>.json`. Removing the now-possibly-empty container is optional — your call, not the skill's.
...
Call it as `manualMigrationChecklist(profileLike, oldEntry, newEntry, vaultRelChaptersDir, provenanceActive)`. The fifth argument is **required** (1.12.0, no default) whenever the entry's delta kind is not null: the function throws rather than silently returning an incomplete checklist if it is omitted or is not a real `true`/`false`. Pass this run's actual W1 ownership outcome — `capture-record.mjs`'s `assertProvenanceOwnership`/`openCaptureRun` result, active iff `ownership.ok` and not `ownership.skip` — never a value inferred from whether `.provenance/` happens to exist on disk (a profile that ran active once and later acquired an overlapping `capture.output_dir` still has a populated tree from a PRIOR run). Passing `false` is a legitimate, explicit answer for a skipped run and reproduces the pre-1.12.0 checklist and halt text byte-for-byte; omitting the argument is not the same thing and is refused outright, because it is exactly the shape of defect that once let an ACTIVE migration's provenance-record move go missing from both the checklist below and the rendered halt text with nothing red to catch it.
...
- non-heading index: verification applies to files for which the fixed-probe writer call returns `kind === 'inserted'`, `kind === 'present'`, or `kind === 'unwritable'` with `field === 'group_title'`, and which hold exactly one selected-target match (a row inside a closed leading frontmatter block is not a match at all). There, the #330 placement verifier (`verifyNonHeadingPlacement`) checks the container labeling this chapter's entry against the current `group_title` — the fact holds on `ok`, while `misplaced` or `inconsistent` re-halts (see "Nested-list automation limits" in `static-md.md`/`obsidian-vault.md`). Explicit confirmation remains for `unverifiable`, which covers every native/YAML MkDocs config and every Markdown nav file outside the class — a wildcard, an ordered list, `<!--nav-->`, or a `group_title` that is not a plain label — this is this manual-migration checklist's own confirmation for a completed title move, a different flow from the adapters' establishment-time non-heading placement check ("INDEX wiring"/"Grouped index wiring" in `static-md.md`/`obsidian-vault.md`), which requests none on its own `unverifiable` outcome; the "Nested-list automation limits" cross-reference above points only at the verified-class definition the two flows share, not at that section's confirmation behavior.

**Grouped entry removed** (`newEntry` is null):

- the old chapter path is gone and the old asset directory is gone;
- **the provenance record fact (1.12.0), when this run's W1 ownership outcome was active**: its record at `<publish.chapters_dir>/.provenance/chapters/<old-group>/<slug>.json` is gone; a skipped run has no provenance tree and this fact does not apply to it;
- no index line targets the old path — for path-link indexes, no line matches the old relative expected target, exactly as in the group-change proof above; for wikilinks mode, no line matches the old entry's **qualified** target either (same `currentIndexExpectedTarget` formula, computed against the old entry) AND no line carries the old entry's **legacy bare** `[[<old slug>]]` spelling under the container titled the OLD `group_title` (from the halt record below) — unlike the group-change case there is no current line to protect, so both halves of the fact simply hold once the old container carries neither spelling, or hold trivially when the old container is gone entirely; non-heading indexes fall back to this checklist's own explicit confirmation, same as the group-changed fact above — a different flow from the adapters' establishment-time placement check, which requests none on `unverifiable`;
...
The mitigation is visible, not silent: on **every** run — not only an `anyGroup` manifest, widened in 1.12.0 because a flat-only handbook can now also accumulate an orphaned provenance record (the grouped-removal recipe's step 7 above never reaches a flat removal) — print a non-blocking stale-artifact advisory listing chapter files under `publish.chapters_dir` and asset directories under `capture.output_dir` that are not derivable from the current manifest, plus, when this run's W1 ownership outcome is active, provenance records under `<publish.chapters_dir>/.provenance/chapters/` with no manifest entry — a plain set-diff over every entry's `chapterRelPath`/`chapterAssetDir`/derived record path against what is actually on disk. This is a WARNING with a pointer back to this recipe, **never an auto-delete**, for the chapter/asset case as before and for the provenance case too: an audit artifact is precisely the one thing this advisory must not silently remove, so an orphaned record is listed, never deleted.
...
This manifest change requires manual group migration (not automated in 1.5.0):
  <slug>: <old_chapter_path> -> <new_chapter_path>; assets <old_asset_dir> -> <new_asset_dir>; record <old_record_path> -> <new_record_path>
  <slug>: container title '<old_title>' -> '<new_title>'
  <slug>: removed — delete <old_chapter_path>, <old_asset_dir>, its index line, and its record <old_record_path> (was under container '<old_title>')
...
**The `; record <old_record_path> -> <new_record_path>` fragment on the first form, and the `, and its record <old_record_path>` fragment on the third, are each present only when this run's W1 ownership outcome was active (1.12.0).** A skipped run has no provenance tree, so there is no record path to name, and the fragment is omitted entirely rather than printed with an empty or placeholder value — omission is the correct rendering for a profile that never owned a `.provenance/` tree, not a defect in the halt. `<old_record_path>` and `<new_record_path>` are always the full derived path — `<publish.chapters_dir>/.provenance/chapters/<group>/<slug>.json` — never the relative tail, the same convention every other path in this halt already follows.
