...
- non-heading index: no present-line placement verifier runs, and raw string presence is gameable (a title that shrank to a substring of the old label, or that string appearing in unrelated nav text, would be vacuously "present"), so the fact is EXPLICIT USER CONFIRMATION that the container labeling this chapter's entry now reads the current `group_title`.
