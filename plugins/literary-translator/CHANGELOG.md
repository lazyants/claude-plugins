...
- **An uncomputable draft sha1 no longer mints a terminal verdict.** `current_sha1 is None` now
  ...
  names what was refused (segment_dispatch_driver.py:3480-3486; the raise itself is
  segment_dispatch_driver.py:751-752). Chosen for the operator's sake, but
...
- **`_cap_still_binds_what_was_reviewed()` narrows the cap-write race; it does not close it.** The
  ...
  **Re-selected is not re-dispatched, and one case turns on that.** A `stale` segment is in the
  ...
  `.ever_converged` marker (select_segments.py:2048-2054) and, on `SENTINEL_PRESENT`, refuses the
  dispatch unless the operator passes `--allow-retranslate-converged` (select_segments.py:2352);
  on `SENTINEL_AMBIGUOUS` it refuses earlier still, and that one the flag does not clear
  (select_segments.py:2057-2064). So a reclassified `stale` heals only with an operator in the loop
