...
Nothing re-translates, and that is a property of WHICH files this release touches rather than of how small the edits are. `select_segments.py` is an ORCHESTRATION member, not one of the 17 `PLUGIN_BUNDLE_MEMBERS`, so no cache key moves and no converged segment re-stales; `SKILL.md` and the tests are in no hashed tuple at all. What does move is `orchestration_bundle_hash`, so a refreshed root's next run is a fresh `RUN_ID` reporting `resume: false` — the ordinary cost of any change to this script. The membership this rests on is now pinned by a test rather than asserted here, because the first draft of this very paragraph claimed the same exemption for `resume_setup.py`, which does not have it.
