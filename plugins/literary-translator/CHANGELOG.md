...
- **Evidence is codepoints, never glyphs.** Sampled tokens and histogram keys are emitted as escapes. A bidi terminal renders a corrupted RTL token identically to an intact one — a finding on this very book was filed and then retracted for exactly that reason — so evidence meant to be adjudicated cannot be rendered text.
...
Suite: 23 new tests, every one watched failing against a deliberate mutation of the code it pins.
