...
Nothing re-translates. `verbatim_census.py` is not a `PLUGIN_BUNDLE_MEMBERS` entry and not an orchestration member, so neither `plugin_bundle_hash` nor `orchestration_bundle_hash` moves, no converged segment goes stale, and no resume identity changes. Step 0a copies the new script into a durable root at the next re-scaffold. `tests/verbatim_census.test.py` adds 38 test functions.
## 1.41.0 — 2026-08-22
