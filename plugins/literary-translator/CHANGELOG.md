...
- **The sentinel name is no longer published until the bytes behind it are durable.** Pre-existing
  ...
  write, `fsync`, or close left an empty regular file behind, the next run classified that residue
  ...
  still never touched (`os.rename()` would have clobbered it). Every failure before the link removes
