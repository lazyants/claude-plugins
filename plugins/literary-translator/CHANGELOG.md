...
78 new tests across `tests/person_registry_prep.test.py` and `tests/person_registry_build.test.py`, driving the shipped script as a subprocess against a real durable root — one carrying all three occurrence origins, an embedded verse whose rendered half and literal gloss are both delivered, a footnote, two occurrences of one name inside a single paragraph, and a nested footnote the renderer never emits.
