...
The record carries the evidence a later claim will need and this one lacked, including the review the claim voids and the cache-key movement across it (claim_record.py:417-426).
...
The evidence scan is now a one-shot snapshot taken before this invocation writes anything, and every consumer reads that one snapshot. Step 3's verdict is a property of the tree as this invocation FOUND it, not as it left it. This is what makes the single-phase claim possible at all: without it, resolving a run id before selection would launder the gate.

### Hardening the write path

The claim record and the re-stamped draft are both fsynced, and so are their DIRECTORIES (claim_record.py:484-536). Without the directory sync, a power loss could retain the new draft token while losing the claim record — token-without-record, the one state the refusal cannot catch, because it sees nothing and reads "unclaimed". A sync failure fails the write rather than reporting a durability it did not establish.

`claimed_path()` validates its run id and raises rather than building a path (claim_record.py:238-266, claim_record.py:187-233). An absolute or `..`-bearing run id used to relocate the lookup out of the durable root, return "absent", and make every guard built on it pass silently, emitting nothing. Validation lives in the function that CONSTRUCTS the path, where no caller can skip it — the writer already validated, and it was the readers that did not.

The draft is staged through a temp file created with `O_CREAT|O_EXCL|O_NOFOLLOW` (select_segments.py:2267). A plain `open()` at a predictable name follows a symlink planted there and truncates its target before the file is ever installed. The staged content is then compared against the hash admission actually gated (select_segments.py:2308-2313), so a draft replaced between admission and stamping is refused rather than silently receiving the claimed token.
