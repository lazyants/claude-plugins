...
The draft is staged through a temp file created with `O_CREAT|O_EXCL|O_NOFOLLOW` (select_segments.py:2426). A plain `open()` at a predictable name follows a symlink planted there and truncates its target before the file is ever installed. The staged content is then compared against the hash admission actually gated (select_segments.py:2467-2472), so a draft whose CONTENT was replaced between admission and stamping is refused rather than silently receiving the claimed token. Stated with its bound, because the qualifier is load-bearing: both comparisons use a hash that projects `dispatch_token` out — it has to, since changing that field is the operation's whole job — so a replacement that moved ONLY the token is invisible to them. That case is handled separately, by refusing to re-stamp a draft another run still owns; the remaining window and the reasoning are recorded in the release's `OPEN.md`.

A claim may not be reasserted over a segment another run has since taken. If the draft's current token names a different run, that run still holds a live claim record, and this run's own record predates this invocation, the re-stamp is refused (select_segments.py:2373-2412). Without it an older run, resumed after a newer one legitimately claimed the same segment, silently took it back — and no content hash could see it, because they all project `dispatch_token` out.

The condition is narrow on purpose, and the narrowness is a consequence rather than a preference. Nothing RELEASES a claim, so a record lives forever; refusing whenever any other run holds one would mean a segment claimed once could never be re-reviewed again, by anyone. So the refusal keys on an OLDER authorization being reapplied, not on a foreign claim existing. A fresh claim over a foreign token is the ordinary change of ownership and still proceeds. `draft_ready.py` tells the two apart too: it no longer reports this state as a lost claim whose remedy is to re-claim, which would be advice to perform exactly the reclaim now refused.
