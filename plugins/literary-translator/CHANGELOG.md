...
What ships instead: `resume_setup.py`'s module docstring and `SKILL.md` now state, where an operator reads them, that `resume: true` asserts digest identity and nothing else; that a refused claim run leaves its own `runs/<RUN_ID>/` and `input.digest` behind for a later invocation to match legitimately; and that the artifact naming which run a segment's work belongs to is that segment's own draft `dispatch_token`, read directly — not the aggregate `dispatching_run_ids`/`run_id_evidence` fields, which report run ids and evidence kinds and not a per-segment ownership map. Both statements are pinned separately, one needle per source file, because a single assertion in the skill would have left the docstring half deletable with nothing red.

### What it costs

Nothing re-translates. `select_segments.py` is an ORCHESTRATION member, not one of the 17 `PLUGIN_BUNDLE_MEMBERS`, so no cache key moves and no converged segment re-stales. What does move is `orchestration_bundle_hash`, so a refreshed root's next run is a fresh `RUN_ID` with `resume: false` — the ordinary cost of any change to this script. `resume_setup.py` changes only its module docstring; that file is not in a hashed tuple either.
