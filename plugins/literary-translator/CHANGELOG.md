...
Every documented operator command runs its script from `${durable_root}/scripts/`, which the pipeline's own passes can write to. `--plugin-root` (LT-409, #412) moved *sibling checker* resolution out of that tree; it never moved the entry point. #582 asked whether that asymmetry is intentional and said that if it is, the fix is a paragraph rather than a command change. It is, and this is the paragraph — recorded in `SKILL.md` beside the `--plugin-root` argument so the next reviewer does not re-derive it.

Rewriting the commands was evaluated first and rejected on evidence, not on cost. It does not close the class:

- **The fix turn is a write-capable deputy.** On any non-clean review `runRound()` calls `callFix()`, which dispatches a plain Claude agent — `agent()` with no `agentType`, so nothing sandboxes it — and instructs it to apply, in full, every entry of a review whose `issue`/`suggest` text codex authored. The only structural check on a finding is that its `loc` contains a colon. That turn is *told* to rewrite only the draft, but the instruction is prose. An untrusted party's free text reaching a write-capable agent is a surface that neither sandboxing codex nor relocating an entry point narrows.
