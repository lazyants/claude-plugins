...
**The obvious remedy does not work, and that is worth writing down.** Both reviewers who found this proposed re-scanning from `m.start() + 1` on a boundary refusal. It was implemented as a mutant and recovers nothing: the nested target begins at the *same* offset as the refused match (`John` and `John Smith` both start at 0), so a `+1` re-scan skips it too. Measured, not reasoned: with that remedy applied, `John Smithson arrived.` still renders unchanged and both of this release's new fixtures stay green. **That is not a global no-op, and the difference matters.** Where the nested target starts LATER than the refused match, a `+1` re-scan does reach it — and reaches the wrong entity: over the prose `JoAnn Marie`, with targets `Ann Marie` and `Marie`, it emits `JoAnn [[…|Marie]]`. `render_obsidian.test.py::test_a_refused_span_is_consumed_so_no_shorter_target_links_inside_it` already pins that and goes RED under the mutant. So the remedy is not merely useless on the case that prompted it — it is the #207 failure everywhere else. Recovery would need per-position fallback to the next-shorter alternative that matches *at that same offset* and passes the boundary test. That is materially more machinery than a scan-position change, for a case whose current outcome is already the safe one, so it is deliberately not implemented.
