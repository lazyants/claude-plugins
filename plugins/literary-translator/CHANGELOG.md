...
- `derive_next_action()` returned `cap_reached` unconditionally the instant the stored review at round label `"final"` was non-clean. The mandatory final round has no round after it to advance to, so that branch read only the review artifact on disk and never asked whether the draft had changed since. The clean branch immediately above it already guards the identical situation on the clean side of the same fork: a review whose recorded `draft_sha1` no longer matches the current draft is re-reviewed at the same round label rather than trusted, because `ledger_update.py`'s `enrich_converged_fields` refuses a convergence write whose `draft_sha1` disagrees with the current draft (ledger_update.py:571-574, `"draft changed since review; cannot record convergence"`) — without a re-dispatch, that refusal repeats on every later invocation, a live-lock. The non-clean final branch had no equivalent guard: it has no convergence write to refuse (`cap_reached`'s own ledger write in `process_segment()` is unconditional and never carries or checks a `draft_sha1`), so the failure was quieter but just as permanent — this function only ever reads `review.json`, so once a segment landed here it stayed capped on every subsequent invocation, even after every finding had been applied by hand and `validate_draft_script` confirmed the result clean. Observed in production: `historiettes-fr-ru/tome1` segments `seg64` and `seg66` each ran their mandatory final review (2 and 7 findings), had every finding applied, and were still permanently reported `outcome="failed", reason="cap"` — together with four older escalations, the sole reason `final_audit.py` reported `project_complete: false` for that book.
- The final branch now reuses the exact discriminator the clean branch already computes (segment_dispatch_driver.py:3049-3051): when the current draft's content sha1 still matches what the stored final review recorded, `cap_reached` is returned exactly as before — a real human decision that a demonstrably-current review still objects to. When it does not match, a fresh review is dispatched at the same `"final"` label instead of capping (`_next_round_label()` treats `"final"` as absorbing — there is no round past it to advance to). Either sha1 being uncomputable stays conservative and still caps, the same tri-state rule the not-clean/not-final branch immediately below already applies to the identical ambiguity.

### Migration

`segment_dispatch_driver.py` is a `PLUGIN_BUNDLE_MEMBERS` entry (cache_key.py:146), so this edit moves `plugin_bundle_hash` again, the same class of consequence its first addition to that tuple carried in 1.18.0. At the next Step 0a bundle refresh, every already-converged mass segment reclassifies as `stale`; the `.ever_converged` sentinel gate `select_segments.py` has carried since 1.18.0 (select_segments.py:1220-1223) refuses to dispatch those segments without `--allow-retranslate-converged` rather than silently retranslating them. The same `plugin_bundle_hash` is also an unconditional input to `resume_setup.py`'s `compute_input_digest()` for both `kind="mass"` and `kind="glossary"` (resume_setup.py:719-722), so this release also invalidates the resume identity of any in-flight run of either kind at its next `resolve_run()`, minting a fresh `RUN_ID` rather than matching the existing digest. `segment_dispatch_driver.py` is not a `DERIVATION_BUNDLE_MEMBERS` entry, so no project routes to `blocked_needs_regeneration` because of this release.

A fresh `RUN_ID` costs more than resume bookkeeping. `translate_dispatch_token(run_id, seg)` is a pure function of `run_id` and `seg` (segment_dispatch_driver.py:2601-2602), so under the new `RUN_ID` `draft_ready.py`'s `--expect-token` check (draft_ready.py:323-328) fails against the `dispatch_token` every existing draft actually carries, since it was written under the old one. In `derive_next_action()`'s `if not draft_ok:` branch, the fix-vs-fresh-translate discriminator is `_matched_review_round_label()` (segment_dispatch_driver.py:2789-2804), which by design matches only a review carrying THIS run's token — a review written under the old `RUN_ID` is not read as fix evidence — so with no matching prior review the branch falls through to `{"action": "translate"}` (segment_dispatch_driver.py:2959). Concretely: any segment with a draft that is not yet converged — `recoverable` and `human_escalation` alike (select_segments.py:1050-1059) — is re-translated from scratch on its next dispatch after upgrading, discarding whatever was applied to that draft by hand. The `.ever_converged` sentinel protects only segments that reached `converged`; this population has none. `seg64` and `seg66` above are in exactly this state: the only path this release opens for them to reach completion is that re-translate, and the findings already applied to them by hand are discarded in the process.
