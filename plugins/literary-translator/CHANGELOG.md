...
- **Do not re-verify a skeptic run's leftover artifacts across this upgrade — re-run `skeptic_setup.py` fresh.** The H1 frozen-input stamps now use a state-tagged hash (`compute_frozen_input_hash_from_state`), so an absent file, an empty regular file and a directory no longer collapse to the same digest. The normal pipeline cannot hit a cross-version mismatch — `skeptic_setup.py` hashes the skeptic scripts into its own code closure, so upgrading already forces a fresh `RUN_ID` before any stale artifact reaches verification. The one real trap is manual: running `--verify-merged` or the new `--check-frozen-inputs` against an OLD run directory's `assignments.json` stamped before this upgrade reads a **false** `frozen_input_mismatch: true` and FATAL-HALTs on a hash-format difference rather than a genuine tamper.
- **Both `producer_input_digest` and `skeptic_input_digest` change format — re-run the suspicion scan and skeptic setup fresh.** Path state (absent / regular / irregular) now enters both digests alongside each file's bytes, not just the H1 stamps. Without it a state-only change was invisible to resume: a sidecar going from absent to a zero-byte regular file left the content `b""` in both cases, so both digests matched, resume proceeded, and it then **overwrote** the H1 stamp with the new state — after which the tripwire compared the mutated file against its own freshly-rewritten stamp and found nothing wrong. Making state part of what "the same inputs" means is what stops resume laundering the mutation. Same invalidation class as every other code-closure change in this release, and free on a fresh run.
