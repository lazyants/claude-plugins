...
- **The sentinel name is no longer published until the bytes behind it are durable.** Pre-existing
  ...
  **Disclosed residual, because six rounds of review were spent learning it cannot be closed
  here.** A run holds one descriptor for `segments/` and checks, at the end, that the path still
  names that directory — reported as `segments_dir_replaced`, in dry runs too. That is a single
  sample, and the census it is meant to vouch for reads each sentinel once by pathname with
  nothing revalidating it afterwards. **A segment can therefore be reported protected when it is
  not, and the result is silent retranslation** — `select_segments.py` gates only the segments it
  finds PRESENT, so a marker that has since gone absent leaves that segment eligible and the
  refusal never fires. Renaming `segments/` is one way to get there and not a precondition: a
  transient mount overlay, network-filesystem failover or snapshot switching, a sync/restore tool
...
- **`_cap_still_binds_what_was_reviewed()` narrows the cap-write race; it does not close it.** The
  ...
  **The refusal is NOT self-healing in every case, and two earlier drafts of this note got the
  ...
  (select_segments.py:1201-1206), outside the default eligible set, and need a human.
  **Re-selected is not re-dispatched, and one case turns on that.** A `stale` segment is in the
  default eligible set (select_segments.py:1228), but Step 1's own sentinel gate then reads its
  `.ever_converged` marker (select_segments.py:1378-1384) and, on `SENTINEL_PRESENT`, refuses the
  dispatch unless the operator passes `--allow-retranslate-converged` (select_segments.py:1428);
  on `SENTINEL_AMBIGUOUS` it refuses earlier still, and that one the flag does not clear
  (select_segments.py:1386-1394). So a reclassified `stale` heals only with an operator in the loop
