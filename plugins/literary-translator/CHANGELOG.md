...
- **The publication window in `reject_review.py` now costs more when it fires.** `os.replace()` publishes the record before `fsync_directory()` can report on it, and ordering cannot close that — 1.23.0's own Known limitations says so. A driver reading in that window used to cost one re-review; it can now record a convergence the operator was told had failed. What it does NOT do is grant something the operator did not decide: the record read there is the complete, validated one their own attested invocation wrote, and what failed is its durability, not the judgement. Re-running the command finds the segment already converged.
