...
stops the run from buying the rest of its batches (#881).** `glossary_dispatch_driver.py`
...
`--resumed-batch-indices`, and `--verdict-dir`, and it refuses either path under a temp root — but
nothing establishes that the resolved durable root is the project's. Nor does anything stop the spend: a
...
W3's dispatch section now says which copy to run, why the invocation path is the selector rather
