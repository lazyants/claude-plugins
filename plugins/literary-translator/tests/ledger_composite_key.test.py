...
def test_durable_root_flag_absent_orphan_copy_fails_self_anchored(tmp_path):
    ...
    --durable-root, cannot succeed via self-anchoring. Asserts the SPECIFIC
    reason -- no `.literary-translator-root.json` ownership marker at the
    self-anchored location -- not merely that some failure occurred: a bare
    "it failed" cannot distinguish this correct refusal from an unrelated
    crash, so a future defect that broke the orphan-copy path for the WRONG
    reason would pass this test silently."""
    orphan_dir = tmp_path / "orphan_location" / "scripts"
    orphan_dir.mkdir(parents=True)
    orphan_script = orphan_dir / "cache_key.py"
    shutil.copy2(SCRIPT_SRC, orphan_script)
    ...
    assert proc.returncode != 0
    assert "ownership marker not found" in proc.stderr, (
        f"expected the orphan copy to fail specifically on its missing "
        f"ownership marker; got a different reason: rc={proc.returncode}\n"
