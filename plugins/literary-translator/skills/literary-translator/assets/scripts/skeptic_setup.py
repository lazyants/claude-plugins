...
try:
    ...
    sys.exit(
        f"skeptic_setup.py: cannot load json_stdout.py from {_JSON_STDOUT_PATH} "
        f"({_json_stdout_exc}).\n"
        "json_stdout.py must be installed alongside skeptic_setup.py under "
...
def main(argv=None) -> int:
    ...
    try:
        ...
        print(dumps_line({"success": False, "error": f"unexpected error: {e}"}))
        return 1
