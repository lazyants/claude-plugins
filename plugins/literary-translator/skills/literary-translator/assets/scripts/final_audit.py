...
try:
    ...
    sys.exit(
        f"final_audit.py: cannot load json_stdout.py from {_JSON_STDOUT_PATH} "
        f"({_json_stdout_exc}).\n"
        "json_stdout.py must be installed alongside final_audit.py under "
...
def ever_converged_path(seg):
    ...
    convergence is recorded) and READ by four scripts: select_segments.py's
    #409 Step 1 re-translate gate, backfill_ever_converged.py's
    already_sentineled scan, assemble.py's #491 machinery-only stale
    carve-out, and this module's own carve-out below -- see that writer's
    docstring for why a separate durable file exists rather than reading the
    (mutable) ledger status. Filename convention restated here rather than
    ...
    The PRESENCE TEST, however, is no longer restated: all five scripts now
    route it through one duplicated-verbatim predicate,
    classify_ever_converged_sentinel() below, because the three `.exists()`
    readers it replaced were free to disagree with the EEXIST-trusting
    writer and with each other, and two of them did."""
...
# ledger_update.py, select_segments.py, backfill_ever_converged.py and
