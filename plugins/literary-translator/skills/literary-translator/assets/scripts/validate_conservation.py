...
try:
    ...
    sys.exit(
        f"validate_conservation.py: cannot load json_stdout.py from {_JSON_STDOUT_PATH} "
        f"({_json_stdout_exc}).\n"
        "json_stdout.py must be installed alongside validate_conservation.py under "
...
def run_wrapper_conservation(manifest):
    ...
    if not conservation_cfg:
        ...
        print(dumps_line({"defects": [], "skipped": True}))
        return True
    ...
    print(dumps_line({"defects": defects}))
    return not defects
...
def run_output_coverage():
    ...
    print(dumps_line(output_doc, allow_nan=False))
    return True
