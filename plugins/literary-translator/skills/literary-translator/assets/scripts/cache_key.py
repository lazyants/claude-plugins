...
try:
    ...
    sys.exit(
        f"cache_key.py: cannot load json_stdout.py from {_JSON_STDOUT_PATH} "
        f"({_json_stdout_exc}).\n"
        "json_stdout.py must be installed alongside cache_key.py under "
...
PLUGIN_BUNDLE_MEMBERS = (
    ...
    # segment_dispatch_driver.py and select_segments.py -- six members of THIS
    # tuple -- now load.
    # Registered for the same transitive-import invisibility that forced
    # canon_senses.py in: this is a literal byte-hash allowlist, so a change to
    # a member's imported dependency is otherwise invisible to it. It reaches
    # durable state through cache_key.py, whose printed object select_segments.py
    # parses as the live cache key and the mass-translate template has the agent
    # copy into a ledger payload. NOT DERIVATION_BUNDLE_MEMBERS, even though
    # bootstrap_names.py loads it too: derivation membership means
    # regenerate-before-retranslate, and this module never touches the durable
...
def main() -> int:
    ...
    print(dumps_line(cache_key, indent=2, sort_keys=False))
    return 0
