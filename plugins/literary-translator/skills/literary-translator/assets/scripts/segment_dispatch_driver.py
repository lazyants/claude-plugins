...
def _translate_in_progress_since(dirs: dict, seg: str, review_path: Path) -> bool:
    ...
    (mass-translate-wf.template.js:2446). A fix turn goes through neither,
    ...
    review (mass-translate-wf.template.js:2336), and a segment left
...
def derive_next_action(seg: str, ctx: "DispatchContext") -> dict:
    ...
    if clean and coverage_ok:
        if draft_matches_review:
            ...
            # template.js:2156 -- `rounds: round`, the NUMERIC loop
            # variable, which equals MAXFIX + 1 on the mandatory final call,
            # template.js:2422). Without this, a segment that converges on
...
def _ledger_rounds_value(round_label: str, max_fix_rounds: int) -> int:
    ...
    isFinal) exactly (template.js:2128): `recordLedgerCall(seg, {status:
    "converged", rounds: round, ...})` (template.js:2156) always
    ...
    template.js:2422). So round_label == "final" -> max_fix_rounds + 1;
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "status in_progress, the convergence sentinel present, and a "
            "stored review that is stale against the current draft -- a "
            "reviewed_draft_sha1 left over from an earlier convergence is "
            "neither required nor disqualifying (#796); select_segments.py --help names "
            "the marker file). An admitted id can NEVER be translated by "
