...
try:
    ...
    sys.exit(
        f"glossary_preflight.py: cannot load json_stdout.py from {_JSON_STDOUT_PATH} "
        f"({_json_stdout_exc}).\n"
        "json_stdout.py must be installed alongside glossary_preflight.py under "
...
def main(argv=None) -> int:
    ...
    # --- Step 7: all clear ----------------------------------------------------
    print(dumps_line({"preflight": "ok"}, separators=(",", ":")))
    return 0
