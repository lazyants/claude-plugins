...
# is pinned BYTE-IDENTICAL across five scripts by
# tests/select_segments.test.py::test_sentinel_predicate_is_identical_in_all_
# five_scripts -- so the import has to exist wherever the copy does, or the
...
def ever_converged_path(seg: str, segments_dir: Path) -> Path:
    ...
    The filename convention is stated in FIVE scripts (this one,
    ledger_update.py, final_audit.py, backfill_ever_converged.py,
    assemble.py), restated rather than imported for the bundle-hash reason
    spelled out in classify_ever_converged_sentinel() below -- NOT for the
    "no shared lib between self-contained scripts" convention, which is
    already false in this codebase. tests/select_segments.test.py's
    ...
    pins the population at five -- drift tests, not a second source of
    truth."""
...
# the copy in the other four sentinel scripts (search `SENTINEL_ABSENT` in
# ledger_update.py, final_audit.py, backfill_ever_converged.py and
# assemble.py) -- see classify_ever_converged_sentinel()'s docstring for
# why it is duplicated rather than imported, and which test pins the five
# copies together.
# ---------------------------------------------------------------------------
...
def check_glossary_current(dirs: dict) -> "dict | None":
    ...
    pass (SKILL.md:1654) -- and read its own verdict rather than
...
def load_glossary_config(durable_root: Path) -> dict:
    ...
    (profile_validate.py:999-1000) spells out the identical convention:
    ...
    if (previously_converged or lost_sentinels) and not args.allow_retranslate_converged:
        ...
        if not_yet_converged:
            second_loss = (
                ...
                f"over every selected NOT-yet-converged segment that ACTUALLY "
                f"CARRIES a draft stamped for another run, naming each affected "
                f"id and its owner. That is a SUBSET of this second number, not "
                f"all of it: an id listed here with no draft on disk -- a "
                f"never-started segment, say -- has no token to be foreign, so it "
                f"is not named by that refusal and translates normally. The "
                f"previously-converged ids in the FIRST number are `stale` and "
                f"exempt from the refusal outright, which is exactly what this "
                f"flag authorized. So the second loss is a HALT you must answer "
                f"per affected segment rather than a silent re-translation over a "
                f"hand-applied fix. That refusal is the driver's; a run driven by "
                f"pipeline() instead has no such gate."
            )
