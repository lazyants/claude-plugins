    ...
    if (previously_converged or lost_sentinels) and not args.allow_retranslate_converged:
        ...
        if not_yet_converged:
            second_loss = (
                ...
                f"over every selected NOT-yet-converged draft whose dispatch_token "
                f"names another run -- i.e. over the ids in THIS second number. "
                f"The previously-converged ids in the FIRST number are `stale` and "
                f"exempt from that refusal, which is exactly what this flag "
                f"authorized -- "
                f"naming each id and its owner, so the second loss is a HALT "
