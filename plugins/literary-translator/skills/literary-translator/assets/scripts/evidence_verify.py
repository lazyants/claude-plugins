...
try:
    from occ_index import production_occurrences, _run_span_keys, attributable_spans
    ...
    sys.exit(
        ...
        "evidence span is bound against, and attributable_spans(), the #927 "
        "Hebrew-pointing refusal applied at the same grouping site. Re-run Step 0a, "
        "or verify the plugin install is not corrupted."
    )

# No direct bootstrap_names import beyond mask_sentinels() below: this module
# reaches the #238/#241 match key ONLY through occ_index's `_run_span_keys()`
# above, which owns the one valid construction of it (`bootstrap_names.
# span_match_keys()`). Importing `fold_match_key` here as well is what
# previously let this file build the key its own way and drift from
# `occ_index.index_manifest()` -- the drift #243 and this release both had to
# close. occ_index.py's own guarded import carries the install-integrity
# check for bootstrap_names.py.

try:
    from bootstrap_names import mask_sentinels
except ImportError as exc:
    sys.exit(
        ...
        "${durable_root}/scripts/ -- it supplies mask_sentinels(), the #927 "
        "same-length substitution attributable_spans() aligns Hebrew pointing "
...
def _group_production_spans_by_name(block_text: str, language_config,
    ...
    #927: parity with `occ_index.production_occurrences()` is kept -- the
    same single-form Hebrew-pointing rule (`attributable_spans()`) is
    applied here too, so a span whose pointing contradicts the one
    non-colliding `source_form` it resolved to is withheld from it here
    exactly as it would be from a direct `production_occurrences()` call."""
    ...
    masked = mask_sentinels(block_text)
    grouped = {}
    for key, spans in folded_spans_by_key.items():
        ...
        grouped[members[0]] = attributable_spans(masked, spans, [members[0]])
    return grouped
