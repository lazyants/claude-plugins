...
| `ELISION_RE` | `string \| null` | Required (non-null) iff `has_elision: true` — an iff enforced at the W3 gate by `language_smoke_report.py`'s loader, which refuses `has_elision: false` beside a non-EMPTY pattern (#116) -- `""` is inert and stays accepted. (`bootstrap_names.py` rejects only the `true` + missing half.) A regex with **exactly 2 capture groups**: group 1 is the elided article's own remnant, group 2 is the name-initial remainder. `null` when `has_elision: false`. |
| `name_inventory` | `string[] \| null` (optional) | Project-local list of full native-script name forms (multi-token allowed, e.g. a two-word Hebrew name), matched as complete forms — not assembled from token runs — to caseless/multiword-surface candidates whose script has no ASCII/`Lu` initial-cap signal (Hebrew letters are Unicode category `Lo`, invisible to the capitalization gate). Absent or `null` in every preset below; none of the shipped Latin-script files set it. |
...
part of this contract — a config that needs to reject elided-contraction
...
| `he.json` | **Unverified starting point (uncased script).** | Hebrew letters are Unicode category `Lo` (no case distinction), so `bootstrap_names.py`'s `Lu`-gated capitalization run never fires. Two consequences: (1) `PARTICLES` is `[]` **by design** — a particle list bridges two *capitalized* tokens, which Hebrew never produces, so any entry would be inert; and (2) a shipped `he.json` **alone surfaces zero native-script name candidates** — real Hebrew names appear only when a project supplies a `name_inventory` override (the optional fifth key above), matched as complete forms rather than assembled from a capitalization run. `STOPWORDS` is a curated ~40-word list of **standalone, whitespace-delimited** Hebrew function words (pronouns, conjunctions, standalone prepositions/adverbs) — deliberately **not** the single-letter proclitics `ה`/`ב`/`כ`/`ל`/`מ`/`ש`/`ו`, which orthographically fuse onto the next word and never stand alone, so they would be dead weight in the one Hebrew consumer: `final_audit.warn_foreign_remainder`, which whitespace-splits the target draft to flag untranslated Hebrew remnants. `has_elision: false` — Hebrew has no French/Italian article-apostrophe elision. **Not smoke-tested against any real book.** |

**These four are unverified starting points, expected to grow — the first
...
only became "proven" after running correctly over an entire real book). For
`he.json` the growth surface is `STOPWORDS` (and any project's own
`name_inventory` override) only — its empty `PARTICLES` is a design invariant
of an uncased script, not a stub awaiting entries.
