...
function translatePrompt(seg) {
  ...
  lines.push("Translate every block, footnote, and verse this segpack contains. Copy every placeholder sentinel (e.g. ⟦FNREF_N⟧, ⟦VERSE_...⟧) byte for byte, in its correct position in the sentence -- never translate, drop, or reword a sentinel itself. Any embedded third-language text (Latin, an older form of the source language, or similar) gets an in-text gloss BY DEFAULT -- but style_bible.md section E's embedded-third-language-convention block states this project's own convention for exactly this, and where it says anything else THAT BLOCK GOVERNS and this sentence does not: romanize without a gloss, translate outright, or keep no source-script text in the target at all. Read that block and apply it as written. What holds either way: never a notes-only translation, because notes never reach the reader. The segpack's canon_map gives each already-canonized name's frozen canonical target form (source form -> target form): render each such name using that target form's stem/spelling, declined as the target grammar requires -- a correctly inflected form of the canonical stem is correct, never a verbatim copy of the citation form where grammar needs another case. For a new_names entry not yet in canon_map, choose a reasoned rendering and flag it in notes as NEW:. The segpack's split_names lists any source form adjudicated as a HOMONYM SPLIT -- one spelling denoting two or more distinct referents, each sense given with a disambiguator. A split form has no frozen target form and never will (that is what a homonym cannot have), so it appears in neither canon_map nor new_names: decide per occurrence which sense the passage carries, render accordingly, and flag your rendering in notes as NEW: naming the sense you chose." + " Entity markup, when this project declares it: before you wrap a name in a declared tag, read " + ROOT + "/entity_markup_untaggable.json if it exists. Each entry names a canon target form that assemble.py refuses as a marked span's LABEL under the tags listed -- the label is the span's ref attribute when it carries one, otherwise the tagged text itself, compared NFC-normalized, so a form that differs only in Unicode composition is the same form -- because two or more canon entries own that form. So leave such a name untagged, and never invent a ref just to escape the list: that identity call belongs to the operator (canon_link_groups.json, or a ref convention style_bible.md already states), not to a translation turn; a span whose ref names a different identity under the project's own stated convention is not affected. If the file is absent, nothing is excluded.");
...
function reviewDispatchPrompt(seg, roundLabel) {
  ...
  lines.push("Canon-name fidelity specifically: the segpack's canon_map gives each already-canonized name's frozen canonical target form. Flag a canon name ONLY if the draft renders a different name, a different transliteration of the canonical stem, leaves a canonical name untranslated, or swaps an epithet for a real surname -- a correctly inflected/declined form of the canonical stem is CORRECT and must NOT be flagged." + " Entity markup, when this project declares it: " + ROOT + "/entity_markup_untaggable.json, if it exists, lists canon target forms that assemble.py refuses as a marked span's LABEL (its ref attribute when present, otherwise the tagged text, compared NFC-normalized) under the tags listed -- two or more canon entries own the form. Never raise a finding asking for a tag whose label would be a listed form, and never read such a form's lack of a tag as an inconsistency with a tagged sibling form -- the list, not the sibling, decides. A span whose label IS a listed form under a listed tag is a valid finding. If the file is absent, this excludes nothing.");
...
function fixPrompt(seg, round, revObj) {
  ...
  // #541 -- THE PREVIOUS ROUND'S VERDICT. The loop could reverse its own applied fix at
  // one loc on consecutive rounds and nothing noticed: each review is a --fresh thread, so
  // round N+1 cannot know round N moved that locus, let alone why, and the canonical
  // review.json is atomically overwritten by the next round's promote, so the earlier
  // verdict was destroyed before this turn ever ran. codex_job.py now keeps it at
  // segments/.prev_review.<seg>.r<label>.json, keyed by that verdict's OWN round label.
  //
  // Emitted for round >= 2 ONLY -- a round-1 prompt gets no path, no token and no
  // adjudication instruction, so a stray r0 artifact has nothing to be read into.
  //
  // THE RECORD IS CONTEXT, NEVER AUTHORITY, and that is the property this whole design
  // rests on rather than a caveat on it: every verdict at one round label mints an
  // identical dispatch_token (reject_review.py's own docstring), so no record can prove it
  // is the exact instance that preceded this round. Nothing here may be applied, refused or
  // reinstated on the record's word; each of those clauses is pinned by a test. That is
  // also what makes an archived verdict an operator has SET ASIDE via reject_review.py
  // harmless: the rejection record is a separate artifact and stays the authority, while
  // this copy of the rejected bytes cannot resurrect anything.
  //
  // The reversal rule deliberately does NOT add a refusal ground. A regressive suggest is
  // just a suggest you do not apply -- the paragraph above already says to fix a real issue
  // another way -- and discarding a substantiated ISSUE because its remedy is regressive
  // would contradict that rule and suppress a valid finding.
  if (Number.isInteger(round) && round >= 2) {
    const prevLabel = round - 1;
    const prevPath = ROOT + "/segments/.prev_review." + seg + ".r" + prevLabel + ".json";
    const prevToken = RUN_ID + ":" + seg + ":r" + prevLabel;
