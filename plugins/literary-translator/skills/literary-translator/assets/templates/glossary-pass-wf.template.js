...
function batchDispatchPrompt(batch, attempt, rejectionReason, sandboxOutPath) {
  ...
  lines.push("research_mode = " + RESEARCH_MODE + ". If it is \"offline\": basis:\"established\" is forbidden outright for every candidate in this batch, with no exception -- use basis:\"transliterated\" when the fixed practical-transcription rule in " + ROOT + "/style_bible.md (section C-translit) settles the form, use basis:\"sense_translated\" instead when the candidate is a speaking name with a clean sense-rendering (see the speaking-name rule below -- legal under offline too, since it makes no citation claim at all), or set disposition:\"review_queue\" instead, with a note that starts with the literal prefix \"SOURCE_UNAVAILABLE:\". What offline forbids is the CITATION-BACKED BASIS, never the target form itself: a form this project's own rules already settle needs no citation, so basis:\"transliterated\" carries whatever section C-translit, read with section C's naming rule, settles -- including a widely-used target-language form for a place or person that already has one, where that rule prefers it. basis:\"transliterated\" is not a letter-by-letter obligation overriding the rule it points at. The \"SOURCE_UNAVAILABLE:\" route is for a candidate whose form those rules do NOT settle and that genuinely needs a human's research later, never for a row they resolved. If it is \"live\": basis:\"established\" is allowed, but only together with a real, citable reference source URL -- never a fabricated one; when a citable conventional target form does exist, cite it under established rather than taking it as transliterated from this project's own preference.")
  ...
  lines.push("- source_form: the candidate's own \"name\" field, copied verbatim -- including a machine-truncation marker if it carries one, which is the batch's own key. See glossary_TASK.md for why such a candidate is never accepted. Your fragment is JSON, so mind the wire format: some source languages spell an ASCII double quote INSIDE a word -- the Hebrew, Yiddish and Aramaic abbreviation mark is one -- and an ASCII apostrophe likewise. Inside the JSON string that double quote MUST be written as a backslash followed by a double quote, and the apostrophe is written bare, exactly as it stands. Never double the quote, never write a backslash before the apostrophe (that is not valid JSON at all), and never substitute a look-alike Unicode mark for either: the decoded string has to come back equal to the candidate's own name, or the self-check below refuses the whole fragment -- for a parse error on the first two, and for coverage on the third.")
...
// fragment when its citations cannot stand as they are: they did not RETRIEVE,
// or (#918) a body came back that attests nothing.
//
...
//
// 1.89.0 (#857), widened by #918: the trailing `cause` parameter. THREE
// DISTINCT facts can put an item in this repair, and each makes exactly one of
// the paragraphs below true.
...
//
// #918 adds the third: "duplicate-body". The URL retrieved, and the RETRIEVAL
// BOUNDARY ITSELF -- no reviewer, nobody who read the page -- observed that the
// bytes it returned are identical to the bytes a different URL returned in this
// same batch. So "unusable-source" cannot be reused for it: that paragraph
// attributes the finding to an independent citation reviewer who read the body,
// and on this path nobody did. For the same reason this cause carries the
// ORDINALS of the rows it is about (the trailing `duplicateOrdinals`): the set
// is usually MIXED, and a paragraph written as if every row were a duplicate
// would be as false as the hedge the rule above forbids.
...
function batchRepairPrompt(batch, attempt, failedRows, sandboxOutPath, cause,
                          hostAdvisory, duplicateOrdinals) {
  ...
  // #918: the third cause, and the trailing `duplicateOrdinals` it needs.
  // Assembled through a local variable instead of a wider ternary so that the
  // two paragraphs that already shipped stay BYTE-IDENTICAL -- both are pinned
  // as complete strings by tests/glossary_dispatch_driver.test.py.
  const dupOrdinals = Array.isArray(duplicateOrdinals) ? duplicateOrdinals : []
  let whyTheseRows
  if (effectiveCause === "unusable-source") {
    whyTheseRows = "Exactly the items below had a source URL that DID retrieve when it was fetched through the project's own retrieval boundary, but an independent citation reviewer -- reading the body that actually came back, not just the URL -- found that body is not the document the URL names at all: an application shell, a bootstrap page, or otherwise a body carrying none of the cited content. That review is a judgment about what the page actually contains, not about your original reasoning, but a source that attests nothing must be replaced exactly like one that never retrieved at all."
  } else if (effectiveCause === "duplicate-body") {
    // A duplicate-body repair set is MIXED more often than not (measured: 23 of
    // 45 flagged evidence directories also hold a row that failed for an
    // ordinary reason), so this branch NAMES which rows it is talking about
    // rather than hedging across all of them. The comment above forbids one
    // paragraph covering two different facts, and "this URL did not answer" is
    // simply false of a row that answered. Rows are named by ORDINAL in the list
    // the agent is about to be shown -- never by source_form, which is not
    // unique -- 1-based, said so in the prose, and rendered through
    // JSON.stringify so nothing reaches the prompt raw.
    const remainder = []
    for (let i = 1; i <= failedRows.length; i++) {
      if (dupOrdinals.indexOf(i) === -1) remainder.push(i)
    }
    const parts = ["Positions below are counted from 1: the first item in the list of items further down is item 1."]
    if (dupOrdinals.length) {
      parts.push("The item(s) at position(s) " + JSON.stringify(dupOrdinals) + " each had a source URL that DID retrieve when it was fetched through the project's own retrieval boundary -- and that boundary itself observed that the bytes it returned are byte-for-byte identical to the bytes a DIFFERENT URL returned in this same batch. That is the signature of a site that serves one application shell for every address, so nothing specific to that URL came back and there is nothing there to support the citation. No reviewer read that body and no review is being quoted to you: this is a fact about the response, established locally by the fetcher, not a judgment about your reasoning.")
    }
    if (remainder.length) {
      parts.push("The item(s) at position(s) " + JSON.stringify(remainder) + " are here for the other reason instead: that source URL COULD NOT BE RETRIEVED AT ALL -- the host answered with an error, or the address did not resolve, or the response was refused for its content type. That is equally a fact about the URL, established locally by the fetcher.")
    }
    whyTheseRows = parts.join(" ")
  } else {
    whyTheseRows = "Exactly the items below had a source URL that COULD NOT BE RETRIEVED AT ALL when it was fetched through the project's own retrieval boundary: the host answered with an error, or the address did not resolve, or the response was refused for its content type. That is a fact about the URL, established locally by the fetcher, not a judgment about your reasoning."
  }
  lines.push("THIS IS NOT A REGENERATION. The rest of this batch was decided, its citations were retrieved successfully, and those rows are NOT yours to touch -- they are not even shown to you. " + whyTheseRows)
...
function citationJudgePrompt(batch, attempt) {
  ...
  lines.push("1. IT RESOLVES. The index records this item's outcome as \"fetched\", and the retrieved body is the reference page itself -- not a 404 page, a parked domain, a login wall that hides the whole content, or plainly a different page than the URL promised. An outcome of \"refused:...\" or \"http_error:...\" FAILS this check: nothing was retrieved, so nothing supports the claim. A refusal is not a technicality to be excused -- a citation the boundary would not fetch is a citation nobody can check. An outcome of \"unusable:duplicate-body\" also FAILS this check, for the opposite reason: a body DID come back, but the retrieval boundary observed it is identical to the body a different URL in this batch returned, so it is one shell served for every address rather than the document this URL names, and there is nothing in it to test against checks 2 and 3 either. Do NOT list such an item on the CITATION_SOURCES_UNUSABLE line described below: that signal is for a body YOU read and judged, and this one the boundary classified before you ever saw it.")
  ...
  lines.push("A DIFFERENT state needs naming, because the rule just above does not reach it and reviewers have been improvising their own answers for it (#921). The index records outcome \"fetched\" and truncated false -- the retrieval boundary handed you every byte it read, and nothing was cut -- and yet the page's whole served text is packed onto ONE physical line, long enough that a read of that line delivers far more at once than you can weigh. No size is quoted here, because size does not separate these pages from ordinary ones; their shape does. Say clearly what this state is not. It is NOT truncation: the flag is present and it is false, so truncation is not available to you as a reason and you must not give it as one. It is also NOT, by itself, the unusable-source class described below -- how a server lays its bytes out across lines is a fact about formatting, not about content. If the bytes you actually read independently show the body to be an application shell, a bootstrap page, or otherwise not the document the URL names, that IS the unusable-source class and its rule applies unchanged, packed onto one line or not; what you may never do is infer that class from the packing alone. The disposition for this state is fixed and it is not yours to vary. OPEN the evidence file the index names and judge the bytes you were actually handed. The length of a line is never by itself a reason to decline an item, and support sitting on an ordinary line must be judged on that line however long some OTHER line in the same file happens to be. If, having read it, the support for checks 2 and 3 is not in what you were handed, FAIL that item exactly as you would fail any other unsupported one -- and give as your reason that the page's text is packed onto a single oversized line, so its support could not be reached in one read. Naming that, precisely as naming truncated:true does, is what sends the next attempt to a smaller or more specific page for the same claim instead of hunting for a fault in a source nobody has shown to be faulty. The two improvisations this rule exists to end are both forbidden: approving an item because the page probably says so somewhere in the part you did not read, and declining to look at all because the file is large.")
