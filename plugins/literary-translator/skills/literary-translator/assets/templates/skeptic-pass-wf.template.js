...
function batchDispatchPrompt(batch) {
  ...
  // The codex:codex-rescue forwarder this prompt is dispatched to chooses
  // foreground or background by its own heuristic unless the request states
  // one, and a foreground choice runs the codex turn inside that forwarder's
  // single Bash call -- so the awaited dispatch below would block for the
  // whole turn and die with the call. The WAIT step below is written for the
  // other shape: a detached job that outlives the awaited call. Stating the
  // routing here is what makes that a request rather than a guess.
  ...
  lines.push("This command schema-validates your fragment and rejects it outright if its shape, its assignment_id/source_form pairing, or its coverage of this batch's assigned entities is wrong. It names the offending items, but a BOUNDED number of them (#360): a large rejection lists several of them and ends with one line saying how many more there are -- so fix each one named, re-run, and expect the next run to name the next batch of them, until it prints a line with \"success\": true. A rejected run changes nothing on disk, so retrying after a rejection is safe; STOP at the FIRST \"success\": true and do not run the command again after that, because the successful run is the one that rewrites your fragment and every further run is a wasted call that tells you nothing new.")
