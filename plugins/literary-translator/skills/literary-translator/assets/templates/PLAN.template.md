...
`verse_policy.mode`, `engine.max_fix_rounds`, `output.target`); and the worth-it rationale for that
choice -- why the chosen tier's cost (or lack of it) is justified for this project's actual goal, not
assumed by default.
<!-- LT_REQUIRED_FILL_END -->
